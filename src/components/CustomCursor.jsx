...
const CustomCursor = () => {
  ...
  useEffect(() => {
    ...
    if (!isMobile) {
      ...
      const handleMouseOver = (e) => {
        if (
          e.target.hasAttribute('data-no-cursor-effect') || 
          e.target.closest('[data-no-cursor-effect]')
        ) {
          return; 
        }
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-[9999]">
      <motion.div
        className="fixed rounded-full bg-gray-600/60 backdrop-blur-sm"
        style={{
          x: cursorX,
          y: cursorY,
          translateX: "-50%",
      ...
      {isClicking && (
        <motion.div
          className="fixed text-white font-mono pointer-events-none select-none"
          style={{
            x: cursorX,
            y: cursorY,
            translateX: "-50%",
